...
def apply_noise(img, noise_type, intensity_range, contrast):
    img = add_noise(img, noise_type, intensity_range, contrast)
    return img
...
class COCO(Dataset):
    ...
    def two_reference_task(self):
        ...
        other_idx = np.random.randint(0, self.num_images)
        other_filename = self.file_list[other_idx]
    ...
    def get_image_pair(self):
        ...
        if choice == 0:  # ref-ref
            ...
            self.same_pair = True
        elif choice == 1:  # ref-other
            reference_filename, other_filename = self.same_different_not_rand()
            self.same_pair = False
        elif choice == 2:  # other-ref
            other_filename, reference_filename = self.same_different_not_rand()
            self.same_pair = False
        else:  # other-other
    ...
    def __getitem__(self, idx):
        ...
        reference_filename, other_filename = self.get_image_pair()
...
class COCO_RNN(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label2 = label2
        scrambled_images = []
        for _ in range(self.num_scrambled):
            gray_image = self.generate_gray_image()
            gray_image = self.apply_gray_background(gray_image.numpy())
            scrambled_images.append(gray_image)
        sequence = [img1] + scrambled_images + [img2]
        labels = [label1] + [None] * self.num_scrambled + [label2]
        return sequence, labels, self.same_pair
