...
parser.add_argument('--kernel_decomposition', type=int, default=499)
...
images_dataloader = DataLoader(dataset, batch_size=32, shuffle=False,)
...
def register_test_loss(model, pairs_trains, configs_tasks, iteration, test_loss_per_iteration_dict):
    ...
    for pairs_trains in pairs_train:
        ...
        for batch_idx, (img1_batch, img2_batch, label1_batch, label2_batch, same_pair) in enumerate(train_loader):
            ...
            while len(lossess) < iterations: 
                ...
                test_loss_per_iteration_dict = register_test_loss(model, pairs_trains, configs_tasks, iteration, test_loss_per_iteration_dict)
        ...
        average_loss,test_dprime = test(model=model,pairs_trains=pairs_trains,name_test='test1',config_test=configs_tasks[1])
